...
for index, repository in enumerate(repositories):
    try:
        ...
        print("Starting {} repository {}/{}".format(repo_name, index, len(repositories)))
        ...
        branches = manager.get_branches(repo_workspace, repository["repository"]["name"])

        for branch_index, branch in enumerate(branches):
            branch_name = branch["name"]
            print("\t\tChecking out branch {} {}/{}".format(branch_name, branch_index, len(branches)))
            repo.git.checkout(branch_name)
            print("\t\tBranch checkout finished!")

        print("Finishing {} repository".format(repo_name))
    except:
        print("OPS... Something happend... Moving to next repo")
