...
class TestNotImplementedProvider(unittest.TestCase):
    def test_init_sets_provider_name_and_auth_error(self):
        from data.source.remote_sources import _NotImplementedProvider

        provider = _NotImplementedProvider("azure")

        self.assertEqual("azure", provider._provider_name)
        self.assertIn("azure", provider.auth_error)
        self.assertIn("not implemented", provider.auth_error)

    def test_auth_ok_returns_false(self):
        from data.source.remote_sources import _NotImplementedProvider
    ...
    def test_get_user_info_raises_not_implemented(self):
        from data.source.remote_sources import _NotImplementedProvider
    ...
    def test_list_repositories_raises_not_implemented(self):
        from data.source.remote_sources import _NotImplementedProvider
        from utils.errors import ProviderNotImplementedError
    ...
    def test_get_repository_raises_not_implemented(self):
        from data.source.remote_sources import _NotImplementedProvider
        from utils.errors import ProviderNotImplementedError
    ...
    def test_list_branches_raises_not_implemented(self):
        from data.source.remote_sources import _NotImplementedProvider
        from utils.errors import ProviderNotImplementedError
...
@unittest.skipUnless(REQUESTS_AVAILABLE, "requests is required for provider tests")
class TestRemoteSourceErrorHandling(unittest.TestCase):
    @patch("data.source.remote_sources.requests.Session")
    ...
    @patch("data.source.remote_sources.requests.Session")
    def test_bitbucket_handles_invalid_json(self, session_cls):
        from utils.errors import RemoteAPIError
    ...
    @patch("data.source.remote_sources.requests.Session")
    def test_bitbucket_handles_403_status(self, session_cls):
        from utils.errors import AuthenticationError

        session = Mock()
        session.get.side_effect = [_FakeResponse(403, {})]
        session_cls.return_value = session
    ...
    @patch("data.source.remote_sources.requests.Session")
    def test_bitbucket_handles_500_status(self, session_cls):
        from utils.errors import RemoteAPIError
    ...
    @patch("data.source.remote_sources.requests.Session")
    def test_github_handles_request_exception(self, session_cls):
        import requests
        from utils.errors import RemoteAPIError
    ...
    @patch("data.source.remote_sources.requests.Session")
    def test_github_handles_invalid_json(self, session_cls):
        from utils.errors import RemoteAPIError

        session = Mock()
        session.get.side_effect = [
            _FakeResponse(200, {"login": "tester"}),
            _FakeResponse(200, ValueError("malformed")),
        ]
        session_cls.return_value = session
    ...
    @patch("data.source.remote_sources.requests.Session")
    def test_gitlab_handles_request_exception(self, session_cls):
        import requests
        from utils.errors import RemoteAPIError
