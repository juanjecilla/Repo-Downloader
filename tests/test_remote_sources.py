...
@unittest.skipUnless(REQUESTS_AVAILABLE, "requests is required for provider tests")
class TestBitbucketSourceLegacyMethods(unittest.TestCase):
    @patch("data.source.remote_sources.requests.Session")
    def test_get_repo_list_legacy_method(self, session_cls):
        session = Mock()
        session.get.side_effect = [
            _FakeResponse(200, {"username": "tester"}),
            _FakeResponse(
        ...
        source = BitbucketSource("user", "token")
        repositories = source.get_repo_list(workspace="acme")

        self.assertEqual(1, len(repositories))

    @patch("data.source.remote_sources.requests.Session")
    def test_get_repositories_by_permission_legacy_method(self, session_cls):
        session = Mock()
        session.get.side_effect = [
            _FakeResponse(200, {"username": "tester"}),
            _FakeResponse(
    ...
    @patch("data.source.remote_sources.requests.Session")
    def test_get_branches_legacy_method(self, session_cls):
        session = Mock()
        session.get.side_effect = [
            _FakeResponse(200, {"username": "tester"}),
...
class TestProviderErrorHandling(unittest.TestCase):
    @patch("data.source.remote_sources.requests.Session")
    def test_bitbucket_request_json_raises_on_json_parse_error(self, session_cls):
        session = Mock()
        session.get.side_effect = [
            _FakeResponse(200, {"username": "tester"}),
    ...
    @patch("data.source.remote_sources.requests.Session")
    def test_github_request_json_raises_on_json_parse_error(self, session_cls):
        session = Mock()
        session.get.side_effect = [
            _FakeResponse(200, {"login": "tester"}),
            _FakeResponse(200, ValueError("invalid json")),
    ...
    @patch("data.source.remote_sources.requests.Session")
    def test_gitlab_request_json_raises_on_json_parse_error(self, session_cls):
        session = Mock()
        session.get.side_effect = [
            _FakeResponse(200, {"username": "tester"}),
            _FakeResponse(200, ValueError("invalid json")),
    ...
    @patch("data.source.remote_sources.requests.Session")
    def test_bitbucket_raises_remote_api_error_on_http_error(self, session_cls):
        session = Mock()
        session.get.side_effect = [
            _FakeResponse(200, {"username": "tester"}),
    ...
    @patch("data.source.remote_sources.requests.Session")
    def test_github_raises_remote_api_error_on_http_error(self, session_cls):
        session = Mock()
        session.get.side_effect = [
            _FakeResponse(200, {"login": "tester"}),
            _FakeResponse(500, {"error": "server error"}),
    ...
    @patch("data.source.remote_sources.requests.Session")
    def test_gitlab_raises_remote_api_error_on_http_error(self, session_cls):
        session = Mock()
        session.get.side_effect = [
            _FakeResponse(200, {"username": "tester"}),
            _FakeResponse(500, {"error": "server error"}),
...
@unittest.skipUnless(REQUESTS_AVAILABLE, "requests is required for provider tests")
class TestGitHubNormalization(unittest.TestCase):
    @patch("data.source.remote_sources.requests.Session")
    def test_github_list_repositories_normalizes_clone_links(self, session_cls):
        session = Mock()
        session.get.side_effect = [
            _FakeResponse(200, {"login": "tester"}),
            _FakeResponse(
                ...
                [
                    {
                        "full_name": "acme/one",
                        "ssh_url": "git@github.com:acme/one.git",
        ...
        # Note: list_repositories doesn't normalize, only get_repository does
        self.assertEqual(1, len(repositories))


@unittest.skipUnless(REQUESTS_AVAILABLE, "requests is required for provider tests")
class TestGitLabNormalization(unittest.TestCase):
    @patch("data.source.remote_sources.requests.Session")
    def test_gitlab_list_repositories_normalizes_full_name(self, session_cls):
        session = Mock()
        session.get.side_effect = [
            _FakeResponse(200, {"username": "tester"}),
            _FakeResponse(
        ...
        self.assertEqual("acme/service", repositories[0]["full_name"])
