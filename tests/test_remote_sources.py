...
class TestGitLabSource(unittest.TestCase):
    ...
    def test_list_branches_pagination(self, session_cls):
        """Test that list_branches handles pagination correctly."""
        session = Mock()
        session.get.side_effect = [
            _FakeResponse(200, {"username": "tester"}),
            _FakeResponse(
        ...
        self.assertEqual(["main", "dev", "feature/x"], [b["name"] for b in branches])

    @patch("data.source.remote_sources.requests.Session")
    def test_invalid_pagination_header_raises_error(self, session_cls):
        """Test that invalid X-Next-Page header raises RemoteAPIError."""
        from utils.errors import RemoteAPIError

        session = Mock()
        session.get.side_effect = [
            _FakeResponse(200, {"username": "tester"}),
            _FakeResponse(200, [{"name": "main"}], headers={"X-Next-Page": "not-a-number"}),
        ]
        session_cls.return_value = session
...
class TestNotImplementedProvider(unittest.TestCase):
    def test_not_implemented_provider_auth_error(self):
        """Test that _NotImplementedProvider sets auth error."""
    ...
    def test_not_implemented_provider_raises_on_all_methods(self):
        """Test that _NotImplementedProvider raises ProviderNotImplementedError."""
        from data.source.remote_sources import _NotImplementedProvider
...
class TestRemoteSourcesEdgeCases(unittest.TestCase):
    @patch("data.source.remote_sources.requests.Session")
    def test_bitbucket_request_exception_raises_remote_api_error(self, session_cls):
        """Test that Bitbucket handles requests.RequestException."""
        from utils.errors import RemoteAPIError
        import requests
    ...
    @patch("data.source.remote_sources.requests.Session")
    def test_github_invalid_json_raises_remote_api_error(self, session_cls):
        """Test that GitHub handles invalid JSON response."""
        from utils.errors import RemoteAPIError
    ...
    @patch("data.source.remote_sources.requests.Session")
    def test_gitlab_normalizes_repository_payload(self, session_cls):
        """Test that GitLab normalizes repository payload with full_name."""
        session = Mock()
        session.get.side_effect = [
            _FakeResponse(200, {"username": "tester"}),
            _FakeResponse(
    ...
    @patch("data.source.remote_sources.requests.Session")
    def test_bitbucket_legacy_methods_work(self, session_cls):
        """Test that Bitbucket legacy methods still work."""
        session = Mock()
        session.get.side_effect = [
            _FakeResponse(200, {"username": "tester"}),
        ...
        # Test get_repo_list (legacy method)
        repos = source.get_repo_list(paginated=True)
        self.assertEqual(1, len(repos))

        # Test get_repositories_by_permission (legacy method)
        repos = source.get_repositories_by_permission(role="admin")
        self.assertEqual(1, len(repos))

        # Test get_branches (legacy method)
        branches = source.get_branches("acme/one")
        self.assertEqual(1, len(branches))

    @patch("data.source.remote_sources.requests.Session")
    def test_providers_handle_403_forbidden_as_auth_error(self, session_cls):
        """Test that all providers handle 403 as authentication error."""
    ...
    @patch("data.source.remote_sources.requests.Session")
    def test_providers_handle_500_server_error(self, session_cls):
        """Test that providers handle 500 server errors as RemoteAPIError."""
        from utils.errors import RemoteAPIError
