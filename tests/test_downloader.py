...
class TestDownloader(unittest.TestCase):
    ...
    def test_normalize_cli_list_strips_whitespace(self):
        result = downloader.normalize_cli_list(["  a  ,  b  "])
    ...
    def test_classify_repository_failure_api_error(self):
        ...
        exc = RemoteAPIError("api timeout")
        self.assertEqual("api", downloader.classify_repository_failure(exc))

    def test_classify_repository_failure_checkout_keyword(self):
        exc = RepositorySyncError("Failed to checkout branch main")
        self.assertEqual("checkout", downloader.classify_repository_failure(exc))

    def test_classify_repository_failure_fetch_keyword(self):
    ...
    def test_classify_repository_failure_clone_keyword(self):
    ...
    def test_format_failure_counters_shows_non_zero_only(self):
        counters = {"api": 0, "auth": 2, "clone": 1, "fetch": 0, "checkout": 0, "other": 0}
        result = downloader.format_failure_counters(counters)
        self.assertIn("auth=2", result)
        self.assertIn("clone=1", result)
        self.assertNotIn("api=0", result)

    def test_format_failure_counters_empty_returns_none(self):
    ...
    def test_select_working_branches_default_only_with_no_default_name(self):
        branches = [{"name": "main"}, {"name": "dev"}]
        selected = downloader.select_working_branches(
            branches=branches,
            explicit_branch_names=None,
    ...
    def test_select_working_branches_filters_branches_without_name(self):
        branches = [{"name": "main"}, {"other": "value"}, {"name": "dev"}]
        selected = downloader.select_working_branches(
            branches=branches,
    ...
    def test_emit_text_outputs_in_text_mode(self):
        logger = _MemoryLogger()
    ...
    def test_emit_text_does_not_output_in_json_mode(self):
        logger = _MemoryLogger()
    ...
    def test_parser_enforces_negative_repo_retries(self):
        parser = downloader.build_parser()
        args = parser.parse_args(["--repo-retries", "-1"])
