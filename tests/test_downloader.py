...
class TestDownloader(unittest.TestCase):  # pylint: disable=too-many-public-methods
    ...
    def test_normalize_cli_list_filters_empty_strings(self):
        result = downloader.normalize_cli_list(["main,,,dev", " , ", ""])
        self.assertEqual(["main", "dev"], result)
    ...
    def test_classify_repository_failure_other_error(self):
    ...
    def test_format_failure_counters_with_multiple_failures(self):
        counters = downloader.make_failure_counters()
        counters["api"] = 2
        counters["clone"] = 1
    ...
    def test_get_default_branch_name_mainbranch_precedence(self):
        repo = {"mainbranch": {"name": "master"}, "default_branch": "main"}
        self.assertEqual("master", downloader.get_default_branch_name(repo))

    def test_get_default_branch_name_none_if_missing(self):
    ...
    def test_select_working_branches_none_input(self):
        selected = downloader.select_working_branches(
            branches=None,
            explicit_branch_names=["main"],
            branch_patterns=["*"],
    ...
    def test_select_working_branches_ignores_branches_without_name(self):
        branches = [{"name": "main"}, {"other_field": "value"}, {"name": "dev"}]
        selected = downloader.select_working_branches(
            branches=branches,
    ...
    def test_buffered_logger_emit_text(self):
        logger = downloader.BufferedLogger(log_format="text")
        logger.emit_text("test message")
        self.assertEqual(1, len(logger.records))
        self.assertEqual("text", logger.records[0][0])
        self.assertEqual("test message", logger.records[0][1])

    def test_buffered_logger_event(self):
        logger = downloader.BufferedLogger(log_format="json")
        logger.event("test.action", outcome="success", key="value")
        self.assertEqual(1, len(logger.records))
        self.assertEqual("event", logger.records[0][0])
        self.assertEqual("test.action", logger.records[0][1]["action"])
        self.assertEqual("success", logger.records[0][1]["outcome"])
        self.assertEqual("value", logger.records[0][1]["fields"]["key"])

    def test_buffered_logger_close(self):
        logger = downloader.BufferedLogger()
        result = logger.close()
        self.assertIsNone(result)

    def test_flush_buffered_logger_to_target(self):
        buffered = downloader.BufferedLogger()
        buffered.emit_text("message 1")
        buffered.event("action.one", outcome="info")
        buffered.emit_text("message 2")

        target = _MemoryLogger()
        downloader.flush_buffered_logger(buffered, target)

        self.assertEqual(1, len(target.events))
        self.assertEqual("action.one", target.events[0]["action"])

    def test_list_backup_entries_empty_directory(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            entries = downloader.list_backup_entries(tmp_dir)
            self.assertEqual([], entries)

    def test_list_backup_entries_nonexistent_directory(self):
        entries = downloader.list_backup_entries("/nonexistent/path/xyz")
        self.assertEqual([], entries)

    def test_load_config_file_raises_on_invalid_extension(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertIn("extension must be .toml, .yaml, or .yml", str(ctx.exception))

    def test_load_config_file_raises_on_non_dict_payload(self):
