...
class TestHelperFunctions(unittest.TestCase):
    ...
    def test_classify_repository_failure_api_error(self):
        ...
        failure_type = downloader.classify_repository_failure(RemoteAPIError("API timeout"))
    ...
    def test_classify_repository_failure_generic_sync_error(self):
        from utils.errors import RepositorySyncError
        failure_type = downloader.classify_repository_failure(
            RepositorySyncError("Something went wrong")
        )
        self.assertEqual("other", failure_type)

    def test_classify_repository_failure_key_error(self):
        failure_type = downloader.classify_repository_failure(KeyError("missing key"))
        self.assertEqual("api", failure_type)
    ...
    def test_classify_repository_failure_unknown_error(self):
        failure_type = downloader.classify_repository_failure(RuntimeError("runtime issue"))
        self.assertEqual("other", failure_type)
    ...
    def test_format_failure_counters_shows_non_zero_values(self):
        ...
        self.assertEqual("api=2, clone=1", result)

    def test_format_failure_counters_respects_order(self):
        counters = {"api": 1, "auth": 2, "clone": 3, "fetch": 0, "checkout": 0, "other": 0}
        result = downloader.format_failure_counters(counters)
        self.assertEqual("api=1, auth=2, clone=3", result)
    ...
    def test_get_default_branch_name_handles_empty_mainbranch(self):
    ...
    def test_select_working_branches_filters_by_pattern(self):
        branches = [
            {"name": "main"},
            {"name": "release/1.0"},
            {"name": "release/2.0"},
        ...
        self.assertEqual([{"name": "release/1.0"}, {"name": "release/2.0"}], result)
    ...
    def test_select_working_branches_combines_names_and_patterns(self):
        ...
        self.assertEqual([{"name": "main"}, {"name": "release/1.0"}], result)
    ...
    def test_select_working_branches_default_only_returns_empty_when_not_found(self):
        branches = [{"name": "main"}, {"name": "dev"}]
        result = downloader.select_working_branches(
            branches=branches,
            explicit_branch_names=[],
            branch_patterns=[],
            default_branch_only=True,
            default_branch_name="nonexistent",
        )
        self.assertEqual([], result)

    def test_select_working_branches_handles_empty_branches(self):
        result = downloader.select_working_branches(
            branches=[],
            explicit_branch_names=["main"],
            branch_patterns=["*"],
