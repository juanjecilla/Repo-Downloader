...
class TestDownloader(unittest.TestCase):
                    ...
                    with patch(
                        ...
                        with patch("downloader.run_backup", return_value=stats):
                            with patch("downloader.importlib.import_module") as mock_import:
                                git_module = _FakeGitModule()
                                mock_import.return_value = git_module
                                exit_code = downloader.main(
                                    [
                                        "--provider",
                                        "bitbucket",
    ...
    def test_classify_repository_failure_identifies_auth_errors(self):
    ...
    def test_classify_repository_failure_identifies_api_errors(self):
        from utils.errors import RemoteAPIError

        failure_type = downloader.classify_repository_failure(RemoteAPIError("api failed"))
        self.assertEqual("api", failure_type)

    def test_classify_repository_failure_identifies_clone_errors(self):
    ...
    def test_classify_repository_failure_identifies_fetch_errors(self):
        from utils.errors import RepositorySyncError
    ...
    def test_classify_repository_failure_identifies_checkout_errors(self):
        from utils.errors import RepositorySyncError

        failure_type = downloader.classify_repository_failure(
            RepositorySyncError("Failed checkout of branch")
        )
        self.assertEqual("checkout", failure_type)

    def test_classify_repository_failure_handles_generic_sync_errors(self):
        from utils.errors import RepositorySyncError
    ...
    def test_classify_repository_failure_treats_value_errors_as_api(self):
        failure_type = downloader.classify_repository_failure(ValueError("parsing failed"))
        self.assertEqual("api", failure_type)

    def test_classify_repository_failure_treats_key_errors_as_api(self):
        failure_type = downloader.classify_repository_failure(KeyError("missing_field"))
        self.assertEqual("api", failure_type)

    def test_classify_repository_failure_treats_type_errors_as_api(self):
    ...
    def test_select_working_branches_handles_branches_without_name(self):
    ...
    def test_select_working_branches_default_only_returns_empty_when_no_default(self):
        branches = [{"name": "main"}, {"name": "dev"}]
        selected = downloader.select_working_branches(
            branches=branches,
            explicit_branch_names=[],
    ...
    def test_format_failure_counters_shows_non_zero_values(self):
        ...
        self.assertNotIn("auth=0", formatted)

    def test_format_failure_counters_shows_none_when_all_zero(self):
        counters = downloader.make_failure_counters()
        formatted = downloader.format_failure_counters(counters)
    ...
    def test_selected_modes_returns_tuple_for_both(self):
        result = downloader.selected_modes("both")
