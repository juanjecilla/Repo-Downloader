...
class TestNormalizeCliList(unittest.TestCase):
    def test_normalize_cli_list_handles_none_input(self):
    ...
    def test_normalize_cli_list_combines_repeatable_and_comma_separated(self):
...
class TestClassifyRepositoryFailure(unittest.TestCase):
    ...
    def test_classify_repository_failure_remote_api_error(self):
        exc = downloader.RemoteAPIError("request failed")
        self.assertEqual("api", downloader.classify_repository_failure(exc))

    def test_classify_repository_failure_checkout_sync_error(self):
    ...
    def test_classify_repository_failure_fetch_sync_error(self):
    ...
    def test_classify_repository_failure_clone_sync_error(self):
    ...
    def test_classify_repository_failure_generic_sync_error(self):
        exc = RepositorySyncError("Something went wrong")
...
class TestGetDefaultBranchName(unittest.TestCase):
    ...
    def test_get_default_branch_name_mainbranch_takes_precedence(self):
    ...
    def test_get_default_branch_name_returns_none_when_name_missing(self):
        repo = {"mainbranch": {}}
        self.assertIsNone(downloader.get_default_branch_name(repo))
...
class TestSelectWorkingBranches(unittest.TestCase):
    ...
    def test_select_working_branches_combines_names_and_patterns(self):
        ...
        self.assertEqual([{"name": "dev"}, {"name": "release/1.0"}], result)

    def test_select_working_branches_default_only_overrides_all_filters(self):
        branches = [{"name": "main"}, {"name": "dev"}, {"name": "release/1.0"}]
        result = downloader.select_working_branches(
            branches=branches,
            explicit_branch_names=["dev"],
    ...
    def test_select_working_branches_default_only_returns_empty_when_default_missing(self):
        branches = [{"name": "main"}, {"name": "dev"}]
        result = downloader.select_working_branches(
            branches=branches,
            explicit_branch_names=[],
...
class TestMakeAndFormatFailureCounters(unittest.TestCase):
    ...
    def test_format_failure_counters_returns_single_type(self):
        counters = downloader.make_failure_counters()
        counters["clone"] = 3
        self.assertEqual("clone=3", downloader.format_failure_counters(counters))

    def test_format_failure_counters_returns_multiple_types_in_order(self):
        counters = downloader.make_failure_counters()
        counters["clone"] = 2
        counters["auth"] = 1
        counters["fetch"] = 3
        result = downloader.format_failure_counters(counters)
        self.assertEqual("auth=1, clone=2, fetch=3", result)
