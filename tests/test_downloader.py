...
class TestDownloaderEdgeCases(unittest.TestCase):
    ...
    def test_normalize_cli_list_filters_empty_strings(self):
        """Verify normalize_cli_list filters out empty strings."""
        result = downloader.normalize_cli_list(["value1", "", "  ", "value2"])
        self.assertEqual(["value1", "value2"], result)

    def test_normalize_cli_list_splits_comma_separated_values(self):
        """Verify normalize_cli_list splits comma-separated values."""
        result = downloader.normalize_cli_list(["a,b,c", "d"])
        self.assertEqual(["a", "b", "c", "d"], result)

    def test_classify_repository_failure_auth_error(self):
        """Verify classify_repository_failure returns 'auth' for AuthenticationError."""
        failure_type = downloader.classify_repository_failure(AuthenticationError("auth failed"))
        self.assertEqual("auth", failure_type)

    def test_classify_repository_failure_api_error(self):
        """Verify classify_repository_failure returns 'api' for RemoteAPIError."""
        failure_type = downloader.classify_repository_failure(RepositorySyncError("API error"))
        self.assertEqual("other", failure_type)

    def test_classify_repository_failure_clone_error(self):
        """Verify classify_repository_failure detects clone errors."""
    ...
    def test_classify_repository_failure_fetch_error(self):
        """Verify classify_repository_failure detects fetch errors."""
        failure_type = downloader.classify_repository_failure(
            RepositorySyncError("Failed fetching updates")
        )
        self.assertEqual("fetch", failure_type)

    def test_classify_repository_failure_checkout_error(self):
        """Verify classify_repository_failure detects checkout errors."""
        failure_type = downloader.classify_repository_failure(
    ...
    def test_classify_repository_failure_key_error_as_api(self):
        """Verify classify_repository_failure treats KeyError as api error."""
        failure_type = downloader.classify_repository_failure(KeyError("missing field"))
        self.assertEqual("api", failure_type)

    def test_classify_repository_failure_type_error_as_api(self):
        """Verify classify_repository_failure treats TypeError as api error."""
        failure_type = downloader.classify_repository_failure(TypeError("wrong type"))
        self.assertEqual("api", failure_type)

    def test_classify_repository_failure_unknown_error(self):
        """Verify classify_repository_failure returns 'other' for unknown errors."""
        failure_type = downloader.classify_repository_failure(RuntimeError("unknown"))
        self.assertEqual("other", failure_type)

    def test_make_failure_counters_includes_all_types(self):
        """Verify make_failure_counters includes all expected failure types."""
    ...
    def test_format_failure_counters_with_no_failures(self):
        """Verify format_failure_counters returns 'none' when all counts are zero."""
        counters = downloader.make_failure_counters()
        formatted = downloader.format_failure_counters(counters)
        self.assertEqual("none", formatted)

    def test_format_failure_counters_with_mixed_failures(self):
        """Verify format_failure_counters formats non-zero counts."""
        counters = downloader.make_failure_counters()
        counters["auth"] = 2
        counters["clone"] = 1
        formatted = downloader.format_failure_counters(counters)
        ...
        self.assertNotIn("api=0", formatted)

    def test_get_default_branch_name_from_mainbranch(self):
        """Verify get_default_branch_name extracts from mainbranch dict."""
    ...
    def test_get_default_branch_name_from_default_branch_dict(self):
        """Verify get_default_branch_name extracts from default_branch dict."""
    ...
    def test_get_default_branch_name_from_default_branch_string(self):
        """Verify get_default_branch_name extracts from default_branch string."""
    ...
    def test_get_default_branch_name_returns_none_when_missing(self):
        """Verify get_default_branch_name returns None when branch info missing."""
        repo = {"full_name": "acme/repo"}
        branch_name = downloader.get_default_branch_name(repo)
        self.assertIsNone(branch_name)

    def test_get_default_branch_name_returns_none_for_non_dict(self):
        """Verify get_default_branch_name returns None for non-dict input."""
    ...
    def test_select_working_branches_returns_empty_for_empty_input(self):
        """Verify select_working_branches returns empty list for empty input."""
    ...
    def test_select_working_branches_returns_all_when_no_filters(self):
        """Verify select_working_branches returns all branches when no filters."""
    ...
    def test_select_working_branches_filters_by_explicit_names(self):
        """Verify select_working_branches filters by explicit branch names."""
        branches = [{"name": "main"}, {"name": "dev"}, {"name": "feature"}]
        result = downloader.select_working_branches(
            branches=branches,
        ...
        self.assertEqual(["main", "feature"], [b["name"] for b in result])

    def test_select_working_branches_filters_by_patterns(self):
        """Verify select_working_branches filters by glob patterns."""
        branches = [{"name": "main"}, {"name": "release/v1"}, {"name": "release/v2"}]
        result = downloader.select_working_branches(
            branches=branches,
            explicit_branch_names=[],
            branch_patterns=["release/*"],
        ...
        self.assertEqual(["release/v1", "release/v2"], [b["name"] for b in result])

    def test_select_working_branches_default_only_overrides_filters(self):
        """Verify default_branch_only overrides other filters."""
        branches = [{"name": "main"}, {"name": "dev"}]
        result = downloader.select_working_branches(
            branches=branches,
            explicit_branch_names=["dev"],
        ...
        self.assertEqual(["main"], [b["name"] for b in result])

    def test_select_working_branches_default_only_returns_empty_when_missing(self):
        """Verify default_branch_only returns empty when default branch not found."""
        branches = [{"name": "dev"}]
        result = downloader.select_working_branches(
            branches=branches,
            explicit_branch_names=[],
    ...
    def test_select_working_branches_ignores_branches_without_name(self):
        """Verify select_working_branches ignores branches without name field."""
        branches = [{"name": "main"}, {"ref": "broken"}, {"name": "dev"}]
        result = downloader.select_working_branches(
            branches=branches,
            explicit_branch_names=["main", "broken", "dev"],
            branch_patterns=[],
            default_branch_only=False,
            default_branch_name=None,
        )
    ...
    def test_emit_text_prints_when_log_format_is_text(self):
        """Verify emit_text prints messages when log format is text."""
        logger = _MemoryLogger()
    ...
    def test_emit_text_does_not_print_when_log_format_is_json(self):
        """Verify emit_text does not print when log format is json."""
        logger = _MemoryLogger()
    ...
    def test_list_backup_entries_returns_empty_for_missing_directory(self):
        """Verify list_backup_entries returns empty list for non-existent directory."""
        entries = downloader.list_backup_entries("/nonexistent/path")
        self.assertEqual([], entries)

    def test_list_backup_entries_with_no_backups(self):
        """Verify list_backup_entries returns empty list when no backups exist."""
        with tempfile.TemporaryDirectory() as tmp_dir:
            entries = downloader.list_backup_entries(tmp_dir)
        self.assertEqual([], entries)

    def test_list_backup_entries_distinguishes_mirror_and_working(self):
        """Verify list_backup_entries distinguishes mirror (.git) from working copies."""
        with tempfile.TemporaryDirectory() as tmp_dir:
            mirror_path = os.path.join(tmp_dir, "bitbucket", "acme", "repo.git")
            working_path = os.path.join(tmp_dir, "bitbucket", "acme", "repo")
            os.makedirs(mirror_path)
            os.makedirs(working_path)

            entries = downloader.list_backup_entries(tmp_dir)

            mirror_entries = [e for e in entries if e["mode"] == "mirror"]
            working_entries = [e for e in entries if e["mode"] == "working"]
            self.assertEqual(1, len(mirror_entries))
            self.assertEqual(1, len(working_entries))

    def test_parser_defaults(self):
        """Verify parser uses correct defaults when no arguments provided."""
        parser = downloader.build_parser()
        args = parser.parse_args([])
        self.assertEqual("backup", args.command)
        self.assertEqual("bitbucket", args.provider)
        self.assertEqual("both", args.mode)
        self.assertEqual("./backups", args.output_dir)
        self.assertIsNone(args.snapshot_format)
        self.assertFalse(args.dry_run)
        self.assertFalse(args.include_archived)

    def test_resolve_token_with_explicit_none_token_env(self):
        """Verify resolve_token prompts when token_env is None."""
        with patch("getpass.getpass", return_value="prompted-token"):
            token = downloader.resolve_token(None)
        self.assertEqual("prompted-token", token)
