...
class TestDownloader(unittest.TestCase):
    ...
    def test_normalize_cli_list_with_whitespace(self):
        result = downloader.normalize_cli_list(["a, b , c"])
        self.assertEqual(["a", "b", "c"], result)

    def test_normalize_cli_list_mixed_repeatable_and_comma(self):
    ...
    def test_get_default_branch_name_prefers_mainbranch_over_default_branch(self):
        repo = {"mainbranch": {"name": "master"}, "default_branch": "main"}
        self.assertEqual("master", downloader.get_default_branch_name(repo))
    ...
    def test_classify_repository_failure_checkout_error(self):
        exc = RepositorySyncError("Failed checkout of branch main")
    ...
    def test_classify_repository_failure_other_sync_error(self):
        exc = RepositorySyncError("some other sync error")
    ...
    def test_format_failure_counters_single_type(self):
        counters = downloader.make_failure_counters()
        counters["clone"] = 3
        self.assertEqual("clone=3", downloader.format_failure_counters(counters))

    def test_format_failure_counters_multiple_types_in_order(self):
        counters = downloader.make_failure_counters()
        counters["checkout"] = 1
        counters["api"] = 2
        counters["fetch"] = 3
        result = downloader.format_failure_counters(counters)
        # Should be ordered by FAILURE_TYPE_ORDER
        self.assertEqual("api=2, fetch=3, checkout=1", result)
    ...
    def test_select_working_branches_empty_input_returns_empty(self):
        selected = downloader.select_working_branches(
            branches=[],
    ...
    def test_select_working_branches_filters_branches_without_name(self):
        ...
        self.assertEqual("dev", selected[1]["name"])
    ...
    def test_select_working_branches_pattern_matching(self):
        ...
        self.assertEqual(2, len(selected))
        self.assertEqual("release/1.0", selected[0]["name"])
        self.assertEqual("release/2.0", selected[1]["name"])

    def test_select_working_branches_combines_explicit_and_patterns(self):
        branches = [
            {"name": "main"},
            {"name": "dev"},
            {"name": "release/1.0"},
    ...
    def test_run_backup_skips_archived_repositories_by_default(self):
        args = SimpleNamespace(
            workspace=None,
            role="member",
            include_archived=False,
        ...
        class _ArchivedProvider(_FakeProvider):
            def get_repository(self, workspace, name):
                repo = super().get_repository(workspace, name)
                repo["archived"] = True
                return repo

        provider = _ArchivedProvider()
        git_source = _FakeGitSource()
        logger = _MemoryLogger()
    ...
    def test_run_backup_includes_archived_when_flag_set(self):
        args = SimpleNamespace(
            workspace=None,
            role="member",
    ...
    def test_run_backup_skips_repository_without_ssh_url(self):
        args = SimpleNamespace(
            workspace=None,
            role="member",
            include_archived=False,
    ...
    def test_run_backup_handles_provider_returning_none_for_repository(self):
        args = SimpleNamespace(
            workspace=None,
            role="member",
            include_archived=False,
        ...
        class _NoneRepoProvider(_FakeProvider):
            def get_repository(self, workspace, name):
                return None

        provider = _NoneRepoProvider()
        git_source = _FakeGitSource()
        logger = _MemoryLogger()
        ...
        self.assertEqual(1, stats["processed"])
        self.assertEqual(1, stats["failed"])
        self.assertEqual(1, stats["failure_types"]["api"])

    def test_run_backup_handles_provider_returning_none_for_repositories_list(self):
        args = SimpleNamespace(
            workspace=None,
            role="member",
            include_archived=False,
        ...
        class _NoneListProvider(_FakeProvider):
            def list_repositories(self, workspace=None, role="member"):
                return None

        provider = _NoneListProvider()
        git_source = _FakeGitSource()
        logger = _MemoryLogger()

        with self.assertRaises(Exception):  # Should raise RemoteAPIError
            downloader.run_backup(args, provider, git_source, logger=logger)

    def test_main_handles_keyboard_interrupt(self):
        with patch("downloader.acquire_run_lock", side_effect=KeyboardInterrupt):
            exit_code = downloader.main(["--provider", "bitbucket", "--username", "user"])

        self.assertEqual(130, exit_code)

    def test_main_validates_repo_retries_non_negative(self):
        with self.assertRaises(SystemExit) as context:
            downloader.main(
                ["--provider", "bitbucket", "--username", "user", "--repo-retries", "-1"]
            )
        self.assertEqual(2, context.exception.code)  # argparse error
