...
class TestDownloader(unittest.TestCase):
    ...
    def test_main_releases_lock_when_run_finishes(self):
        ...
        class _FakeGitSourceModule:
            class GitSource:
                def __init__(self, key_path):
                    self.key_path = key_path

        def fake_import_module(name):
            if name == "data.source.git_source":
                return _FakeGitSourceModule()
            # Fall back to actual import for other modules
            import importlib
            ...
            with patch("downloader.release_run_lock") as mock_release:
                with patch("downloader.resolve_token", return_value="token-value"):
                    with patch(
                        ...
                        with patch("downloader.importlib.import_module", side_effect=fake_import_module):
