...
class TestDownloader(unittest.TestCase):
    ...
    def test_classify_repository_failure_authentication_error(self):
    ...
    def test_classify_repository_failure_remote_api_error(self):
        failure_type = downloader.classify_repository_failure(RepositorySyncError("Failed to clone"))
        self.assertEqual("clone", failure_type)

    def test_classify_repository_failure_fetch_error(self):
        failure_type = downloader.classify_repository_failure(RepositorySyncError("Failed fetching"))
        self.assertEqual("fetch", failure_type)

    def test_classify_repository_failure_checkout_error(self):
        failure_type = downloader.classify_repository_failure(RepositorySyncError("Failed checkout branch"))
        self.assertEqual("checkout", failure_type)

    def test_classify_repository_failure_generic_sync_error(self):
        failure_type = downloader.classify_repository_failure(RepositorySyncError("Something went wrong"))
        self.assertEqual("other", failure_type)

    def test_classify_repository_failure_key_error(self):
        failure_type = downloader.classify_repository_failure(KeyError("missing_key"))
        self.assertEqual("api", failure_type)

    def test_classify_repository_failure_type_error(self):
        failure_type = downloader.classify_repository_failure(TypeError("type mismatch"))
        self.assertEqual("api", failure_type)
    ...
    def test_format_failure_counters_single_type(self):
        ...
        counters["clone"] = 3
        formatted = downloader.format_failure_counters(counters)
        self.assertEqual("clone=3", formatted)

    def test_format_failure_counters_multiple_types(self):
        counters = downloader.make_failure_counters()
        counters["auth"] = 1
        counters["clone"] = 2
        counters["api"] = 3
        formatted = downloader.format_failure_counters(counters)
        self.assertEqual("api=3, auth=1, clone=2", formatted)
    ...
    def test_get_default_branch_name_prefers_mainbranch_over_default_branch(self):
        repository = {"mainbranch": {"name": "master"}, "default_branch": {"name": "main"}}
        branch_name = downloader.get_default_branch_name(repository)
        self.assertEqual("master", branch_name)

    def test_get_default_branch_name_returns_none_for_missing_branch(self):
    ...
    def test_select_working_branches_empty_branches_list(self):
        selected = downloader.select_working_branches(
            branches=[],
    ...
    def test_select_working_branches_matches_multiple_patterns(self):
        ...
        self.assertIn("feature/y", names)
        self.assertIn("hotfix/z", names)

    def test_emit_text_prints_for_text_format(self):
        logger = _MemoryLogger()
    ...
    def test_emit_text_suppresses_output_for_json_format(self):
        logger = _MemoryLogger()
    ...
    def test_run_backup_skips_archived_repository(self):
        args = SimpleNamespace(
            workspace=None,
            role="member",
            include_archived=False,
        ...
        class _ArchivedRepoProvider(_FakeProvider):
            def get_repository(self, workspace, name):
                return {
                    "full_name": f"{workspace}/{name}",
                    "links": {"clone": [{"name": "ssh", "href": "git@example.com:acme/example.git"}]},
        ...
        provider = _ArchivedRepoProvider()
        git_source = _FakeGitSource()
        logger = _MemoryLogger()
    ...
    def test_run_backup_skips_repository_without_ssh_clone_url(self):
        ...
        class _NoSSHProvider(_FakeProvider):
            def get_repository(self, workspace, name):
                return {
                    "full_name": f"{workspace}/{name}",
                    "links": {"clone": [{"name": "https", "href": "https://example.com/acme/example.git"}]},
                    "is_archived": False,
                    "mainbranch": {"name": "main"},
                }
    ...
    def test_run_backup_working_mode_only(self):
        args = SimpleNamespace(
            workspace=None,
            role="member",
            include_archived=False,
        ...
        logged_actions = [event["action"] for event in logger.events]
        self.assertIn("sync.working.clone", logged_actions)
        self.assertNotIn("sync.mirror.clone", logged_actions)

    def test_run_backup_mirror_mode_only(self):
        args = SimpleNamespace(
            workspace=None,
            role="member",
            include_archived=False,
        ...
        logged_actions = [event["action"] for event in logger.events]
        self.assertIn("sync.mirror.clone", logged_actions)
        self.assertNotIn("sync.working.clone", logged_actions)
