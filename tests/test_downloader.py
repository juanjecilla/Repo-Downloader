...
class TestDownloader(unittest.TestCase):  # pylint: disable=too-many-public-methods
    ...
    def test_main_releases_lock_when_run_finishes(self):
        ...
        def fake_import(module_name):
            if module_name == "data.source.git_source":
                return _FakeGitModule()
            # Use the real importlib for other modules
            return real_importlib.__import__(module_name, fromlist=[""])
    ...
    def test_normalize_cli_list_handles_empty_input(self):
        ...
        normalized = downloader.normalize_cli_list([])
        self.assertEqual([], normalized)

    def test_normalize_cli_list_strips_whitespace(self):
        normalized = downloader.normalize_cli_list([" acme/* ", " other/* "])
        self.assertEqual(["acme/*", "other/*"], normalized)
    ...
    def test_classify_repository_failure_returns_other_for_unknown_error(self):
    ...
    def test_format_failure_counters_returns_none_for_empty(self):
        counters = downloader.make_failure_counters()
        formatted = downloader.format_failure_counters(counters)
        self.assertEqual("none", formatted)

    def test_format_failure_counters_includes_non_zero_values(self):
        counters = {"auth": 2, "clone": 1, "api": 0, "fetch": 0, "checkout": 0, "other": 0}
        formatted = downloader.format_failure_counters(counters)
    ...
    def test_get_default_branch_name_handles_non_dict_input(self):
    ...
    def test_list_backup_entries_returns_empty_for_nonexistent_directory(self):
        entries = downloader.list_backup_entries("/nonexistent/path")
        self.assertEqual([], entries)

    def test_list_backup_entries_identifies_mirror_and_working_backups(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            mirror_path = os.path.join(tmp_dir, "bitbucket", "acme", "example.git")
            working_path = os.path.join(tmp_dir, "bitbucket", "acme", "example")
