...
class TestDownloader(unittest.TestCase):
    ...
    def test_acquire_run_lock_called_by_main(self):
        """Test that main properly acquires and releases the run lock."""
        with patch("downloader.acquire_run_lock", side_effect=RunLockError("lock active")):
            exit_code = downloader.main([
                "--provider",
                "bitbucket",
        ...
        # Should fail with exit code 1 when lock cannot be acquired
        self.assertEqual(1, exit_code)
    ...
    def test_classify_repository_failure_api_error(self):
        ...
        exc = RemoteAPIError("API request failed")
        result = downloader.classify_repository_failure(exc)
        self.assertEqual("api", result)

    def test_classify_repository_failure_sync_clone_error(self):
        exc = RepositorySyncError("Failed cloning repository")
        result = downloader.classify_repository_failure(exc)
        self.assertEqual("clone", result)

    def test_classify_repository_failure_sync_fetch_error(self):
        exc = RepositorySyncError("Failed fetching working repository")
        result = downloader.classify_repository_failure(exc)
        self.assertEqual("fetch", result)

    def test_classify_repository_failure_sync_checkout_error(self):
        exc = RepositorySyncError("error during checkout of branch 'main'")
        result = downloader.classify_repository_failure(exc)
        self.assertEqual("checkout", result)

    def test_classify_repository_failure_sync_other_error(self):
    ...
    def test_format_failure_counters_multiple_types(self):
        ...
        counters["other"] = 1
        result = downloader.format_failure_counters(counters)
        self.assertEqual("auth=2, clone=1, other=1", result)
    ...
    def test_format_failure_counters_preserves_order(self):
        ...
        counters["auth"] = 3
        result = downloader.format_failure_counters(counters)
        self.assertEqual("api=2, auth=3, other=1", result)
    ...
    def test_select_working_branches_pattern_matching(self):
        ...
        self.assertEqual(["feature/login", "feature/signup"], [b["name"] for b in result])

    def test_select_working_branches_with_explicit_filters_skips_branches_without_name(self):
    ...
    def test_select_working_branches_default_only_without_default_branch(self):
        branches = [{"name": "main"}, {"name": "dev"}]
        result = downloader.select_working_branches(
            branches=branches,
            explicit_branch_names=["dev"],
    ...
    def test_make_failure_counters_creates_all_types(self):
        counters = downloader.make_failure_counters()
    ...
    def test_create_provider_raises_for_unknown_provider(self):
        args = argparse.Namespace(provider="unknown")
