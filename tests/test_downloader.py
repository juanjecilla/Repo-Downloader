...
class TestDownloader(unittest.TestCase):
                ...
                with patch("downloader.resolve_token", return_value="token-value"):
                    with patch(
                        ...
                        with patch("downloader.run_backup", return_value=stats):
                            with patch("downloader.importlib.import_module") as mock_import:
                                mock_git_module = _FakeGitModule()
                                mock_import.return_value = mock_git_module
                                exit_code = downloader.main(
                                    [
                                        "--provider",
                                        "bitbucket",
    ...
    def test_normalize_cli_list_expands_comma_separated_values(self):
        result = downloader.normalize_cli_list(["main,dev", "feature"])
        self.assertEqual(["main", "dev", "feature"], result)

    def test_normalize_cli_list_strips_whitespace(self):
        result = downloader.normalize_cli_list(["  main  ", "dev,  feature  "])
        self.assertEqual(["main", "dev", "feature"], result)

    def test_normalize_cli_list_returns_empty_for_none_input(self):
        result = downloader.normalize_cli_list(None)
        self.assertEqual([], result)

    def test_normalize_cli_list_skips_empty_strings(self):
        result = downloader.normalize_cli_list(["main,,dev", " , feature"])
        self.assertEqual(["main", "dev", "feature"], result)

    def test_classify_repository_failure_identifies_auth_error(self):
        exc = AuthenticationError("token expired")
        self.assertEqual("auth", downloader.classify_repository_failure(exc))

    def test_classify_repository_failure_identifies_api_error(self):
        exc = downloader.RemoteAPIError("request timeout")
        self.assertEqual("api", downloader.classify_repository_failure(exc))

    def test_classify_repository_failure_identifies_clone_error(self):
        exc = RepositorySyncError("Failed cloning repository from git@example.com")
        self.assertEqual("clone", downloader.classify_repository_failure(exc))

    def test_classify_repository_failure_identifies_fetch_error(self):
        exc = RepositorySyncError("Failed fetching working repository")
        self.assertEqual("fetch", downloader.classify_repository_failure(exc))

    def test_classify_repository_failure_identifies_checkout_error(self):
        exc = RepositorySyncError("Failed Checkout branch 'main'")
        self.assertEqual("checkout", downloader.classify_repository_failure(exc))

    def test_classify_repository_failure_identifies_other_sync_error(self):
        exc = RepositorySyncError("Unknown git error occurred")
        self.assertEqual("other", downloader.classify_repository_failure(exc))

    def test_classify_repository_failure_treats_key_error_as_api(self):
        exc = KeyError("missing field")
        self.assertEqual("api", downloader.classify_repository_failure(exc))

    def test_classify_repository_failure_treats_type_error_as_api(self):
        exc = TypeError("expected dict, got list")
        self.assertEqual("api", downloader.classify_repository_failure(exc))

    def test_classify_repository_failure_treats_value_error_as_api(self):
        exc = ValueError("invalid JSON")
        self.assertEqual("api", downloader.classify_repository_failure(exc))

    def test_classify_repository_failure_defaults_to_other(self):
        exc = RuntimeError("unexpected error")
        self.assertEqual("other", downloader.classify_repository_failure(exc))

    def test_make_failure_counters_initializes_all_types_to_zero(self):
    ...
    def test_format_failure_counters_shows_only_non_zero_values(self):
    ...
    def test_get_default_branch_name_extracts_from_mainbranch_dict(self):
    ...
    def test_get_default_branch_name_prefers_mainbranch_over_default_branch(self):
        repo = {"mainbranch": {"name": "main"}, "default_branch": {"name": "develop"}}
        result = downloader.get_default_branch_name(repo)
        self.assertEqual("main", result)

    def test_get_default_branch_name_returns_none_when_not_found(self):
    ...
    def test_get_default_branch_name_handles_missing_name_in_dict(self):
        repo = {"mainbranch": {"type": "branch"}}
        result = downloader.get_default_branch_name(repo)
        self.assertIsNone(result)
    ...
    def test_select_working_branches_returns_empty_when_no_branches(self):
        selected = downloader.select_working_branches(
            branches=[],
    ...
    def test_select_working_branches_returns_empty_when_default_only_but_no_default(self):
        branches = [{"name": "main"}, {"name": "dev"}]
        selected = downloader.select_working_branches(
            branches=branches,
            explicit_branch_names=[],
    ...
    def test_select_working_branches_filters_by_explicit_names_only(self):
        branches = [{"name": "main"}, {"name": "dev"}, {"name": "feature"}]
        selected = downloader.select_working_branches(
            branches=branches,
        ...
        self.assertEqual(["main", "feature"], [b["name"] for b in selected])

    def test_select_working_branches_filters_by_patterns_only(self):
        branches = [
            {"name": "main"},
            {"name": "dev"},
            {"name": "feature/x"},
        ...
        self.assertEqual(["feature/x", "feature/y"], [b["name"] for b in selected])
    ...
    def test_select_working_branches_combines_names_and_patterns(self):
        ...
        self.assertEqual(["main", "release/1.0", "release/2.0"], [b["name"] for b in selected])

    def test_select_working_branches_skips_branches_without_name_field(self):
    ...
    def test_create_provider_raises_error_for_unknown_provider(self):
        args = argparse.Namespace(provider="unknown-provider")
        with self.assertRaises(downloader.ProviderConfigurationError) as context:
            downloader.create_provider(args, token="x")
        self.assertIn("Unknown provider", str(context.exception))

    def test_emit_text_prints_message_when_log_format_is_text(self):
        logger = SimpleNamespace(log_format="text")
        buffer = io.StringIO()
    ...
    def test_emit_text_does_not_print_when_log_format_is_json(self):
        logger = SimpleNamespace(log_format="json")
        with patch("builtins.print") as mock_print:
            downloader.emit_text(logger, "test message")
    ...
    def test_parser_defaults(self):
        ...
        self.assertEqual("./backups", args.output_dir)
        self.assertEqual("~/.ssh/id_rsa", args.ssh_key_path)
        self.assertEqual("member", args.role)
        ...
        self.assertIsNone(args.token_env)
        self.assertFalse(args.include_archived)
        self.assertFalse(args.dry_run)
        self.assertFalse(args.default_branch_only)
        self.assertEqual(0, args.repo_retries)
        self.assertFalse(args.force_lock)

    def test_main_validates_repo_retries_positive(self):
