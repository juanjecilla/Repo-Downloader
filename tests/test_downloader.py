...
class TestDownloader(unittest.TestCase):
    ...
    @patch("downloader.run_backup")
    @patch("downloader.importlib.import_module")
    @patch("downloader.create_provider")
    @patch("downloader.resolve_token")
    @patch("downloader.release_run_lock")
    @patch("downloader.acquire_run_lock")
    def test_main_releases_lock_when_run_finishes(
        self, mock_acquire, mock_release, mock_token, mock_provider, mock_import, mock_backup
    ):
        ...
        mock_token.return_value = "token-value"
        mock_provider.return_value = fake_provider
        mock_import.return_value = _FakeGitModule()
...
class TestDownloaderEdgeCases(unittest.TestCase):
    ...
    def test_normalize_cli_list_handles_whitespace_only_values(self):
        result = downloader.normalize_cli_list(["  ", ",  ,", " "])
        self.assertEqual([], result)
    ...
    def test_classify_repository_failure_auth_error(self):
        ...
        result = downloader.classify_repository_failure(exc)
        self.assertEqual("auth", result)

    def test_classify_repository_failure_api_error(self):
        from utils.errors import RemoteAPIError

        exc = RemoteAPIError("API returned 500")
        result = downloader.classify_repository_failure(exc)
        self.assertEqual("api", result)

    def test_classify_repository_failure_sync_error_with_clone(self):
        exc = RepositorySyncError("Failed cloning repository")
        result = downloader.classify_repository_failure(exc)
        self.assertEqual("clone", result)

    def test_classify_repository_failure_sync_error_with_fetch(self):
        exc = RepositorySyncError("Failed fetching working repository")
        result = downloader.classify_repository_failure(exc)
        self.assertEqual("fetch", result)

    def test_classify_repository_failure_sync_error_with_checkout(self):
        exc = RepositorySyncError("Failed checkout operation")
        result = downloader.classify_repository_failure(exc)
        self.assertEqual("checkout", result)

    def test_classify_repository_failure_sync_error_other(self):
        exc = RepositorySyncError("Unknown sync error")
        result = downloader.classify_repository_failure(exc)
    ...
    def test_classify_repository_failure_type_error(self):
        exc = TypeError("Expected dict, got None")
        result = downloader.classify_repository_failure(exc)
        self.assertEqual("api", result)
    ...
    def test_format_failure_counters_single_type(self):
        counters = downloader.make_failure_counters()
        counters["auth"] = 3
        result = downloader.format_failure_counters(counters)
        self.assertEqual("auth=3", result)

    def test_format_failure_counters_multiple_types_ordered(self):
        counters = downloader.make_failure_counters()
        counters["checkout"] = 2
        counters["auth"] = 1
        counters["api"] = 5
        result = downloader.format_failure_counters(counters)
        self.assertEqual("api=5, auth=1, checkout=2", result)

    def test_get_default_branch_name_from_mainbranch(self):
        repository = {"mainbranch": {"name": "master"}}
        result = downloader.get_default_branch_name(repository)
        self.assertEqual("master", result)

    def test_get_default_branch_name_from_default_branch_dict(self):
        repository = {"default_branch": {"name": "main"}}
        result = downloader.get_default_branch_name(repository)
    ...
    def test_get_default_branch_name_prefers_mainbranch(self):
        repository = {"mainbranch": {"name": "master"}, "default_branch": "main"}
        result = downloader.get_default_branch_name(repository)
        self.assertEqual("master", result)

    def test_get_default_branch_name_returns_none_if_missing(self):
        repository = {"full_name": "acme/repo"}
        result = downloader.get_default_branch_name(repository)
        self.assertIsNone(result)

    def test_get_default_branch_name_returns_none_for_invalid_input(self):
        result = downloader.get_default_branch_name(None)
        self.assertIsNone(result)
    ...
    def test_select_working_branches_default_only_returns_empty_if_no_default(self):
        branches = [{"name": "main"}, {"name": "dev"}]
        result = downloader.select_working_branches(
            branches=branches,
            explicit_branch_names=[],
    ...
    def test_select_working_branches_matches_multiple_patterns(self):
        branches = [
            {"name": "main"},
            {"name": "release/1.0"},
            {"name": "release/2.0"},
        ...
        self.assertEqual(["release/1.0", "release/2.0", "hotfix/bug"], branch_names)
    ...
    def test_select_working_branches_combines_names_and_patterns(self):
        ...
        branch_names = [b["name"] for b in result]
        self.assertEqual(["main", "release/1.0"], branch_names)

    def test_emit_text_only_prints_for_text_logger(self):
        logger = _MemoryLogger()
    ...
    def test_parser_rejects_negative_repo_retries(self):
        parser = downloader.build_parser()
        with self.assertRaises(SystemExit):
            with patch("sys.stderr"):
                downloader.main(["--repo-retries", "-1", "--dry-run"])

    def test_run_backup_handles_none_repository_list(self):
        from utils.errors import RemoteAPIError
        ...
        self.assertIn("returned no repository data", str(context.exception))

    def test_run_backup_handles_repository_without_ssh_url(self):
        args = SimpleNamespace(
            workspace=None,
            role="member",
            include_archived=False,
        ...
        class _NoSSHProvider(_FakeProvider):
            def get_repository(self, workspace, name):
                return {
                    "full_name": f"{workspace}/{name}",
                    "links": {"clone": [{"name": "https", "href": "https://example.com/repo.git"}]},
                }

        provider = _NoSSHProvider()
        git_source = _FakeGitSource()
        logger = _MemoryLogger()
    ...
    def test_run_backup_handles_provider_returning_none_repository(self):
        ...
        class _NullRepoProvider(_FakeProvider):
            def get_repository(self, workspace, name):
                return None

        provider = _NullRepoProvider()
        git_source = _FakeGitSource()
        logger = _MemoryLogger()
        ...
        self.assertEqual(0, stats["succeeded"])
        self.assertEqual(1, stats["failed"])
        error_events = [e for e in logger.events if e["outcome"] == "failed"]
        self.assertTrue(any("did not return details" in str(e) for e in error_events))
