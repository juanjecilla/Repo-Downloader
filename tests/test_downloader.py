...
class TestDownloaderEdgeCases(unittest.TestCase):
    ...
    def test_normalize_cli_list_skips_empty_parts(self):
    ...
    def test_classify_repository_failure_api_error(self):
        ...
        error = RemoteAPIError("request failed")
        failure_type = downloader.classify_repository_failure(error)
        self.assertEqual("api", failure_type)

    def test_classify_repository_failure_checkout_error(self):
        error = RepositorySyncError("Failed to checkout branch 'main'")
        failure_type = downloader.classify_repository_failure(error)
    ...
    def test_classify_repository_failure_other_sync_error(self):
        error = RepositorySyncError("Something else went wrong")
        failure_type = downloader.classify_repository_failure(error)
    ...
    def test_format_failure_counters_single_type(self):
        ...
        counters["auth"] = 3
        formatted = downloader.format_failure_counters(counters)
        self.assertEqual("auth=3", formatted)
    ...
    def test_format_failure_counters_multiple_types(self):
        ...
        self.assertIn("clone=5", formatted)
    ...
    def test_get_default_branch_name_returns_none_for_none_input(self):
    ...
    def test_select_working_branches_skips_branch_without_name(self):
        branches = [{"name": "main"}, {"ref": "dev"}, {"name": "feature"}]
        selected = downloader.select_working_branches(
            branches=branches,
    ...
    def test_emit_text_prints_when_logger_format_is_text(self):
        logger = SimpleNamespace(log_format="text")
        with patch("builtins.print") as mock_print:
            downloader.emit_text(logger, "test message")
            mock_print.assert_called_once_with("test message")

    def test_emit_text_does_not_print_when_logger_format_is_json(self):
        logger = SimpleNamespace(log_format="json")
        with patch("builtins.print") as mock_print:
            downloader.emit_text(logger, "test message")
            mock_print.assert_not_called()

    def test_emit_text_handles_logger_without_log_format_attribute(self):
        logger = SimpleNamespace()
        with patch("builtins.print") as mock_print:
            downloader.emit_text(logger, "test message")
            mock_print.assert_called_once_with("test message")

    def test_main_validates_negative_repo_retries(self):
