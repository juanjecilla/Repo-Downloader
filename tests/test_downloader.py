...
class TestDownloader(unittest.TestCase):  # pylint: disable=too-many-public-methods
    ...
    def test_normalize_cli_list_handles_empty_input(self):
        """Test that normalize_cli_list handles None and empty lists."""
    ...
    def test_normalize_cli_list_strips_whitespace(self):
        """Test that normalize_cli_list strips whitespace from values."""
        result = downloader.normalize_cli_list(["  foo  ", "bar,  baz  "])
        self.assertEqual(["foo", "bar", "baz"], result)

    def test_normalize_cli_list_removes_empty_values(self):
        """Test that normalize_cli_list removes empty string values."""
        result = downloader.normalize_cli_list(["foo", "", "bar", "  ", "baz"])
        self.assertEqual(["foo", "bar", "baz"], result)

    def test_classify_repository_failure_handles_all_error_types(self):
        """Test that classify_repository_failure correctly classifies all error types."""
        self.assertEqual("auth", downloader.classify_repository_failure(AuthenticationError("auth")))
        self.assertEqual("api", downloader.classify_repository_failure(RemoteAPIError("api")))
        self.assertEqual("clone", downloader.classify_repository_failure(
            RepositorySyncError("Failed cloning repository from x")
        ))
        self.assertEqual("fetch", downloader.classify_repository_failure(
            RepositorySyncError("Failed fetching working repository")
        ))
        # Note: "checkout" is checked as a substring, but actual messages use "checking out"
        # So this should be classified as "other" unless we use "checkout" without space
        self.assertEqual("checkout", downloader.classify_repository_failure(
            RepositorySyncError("Failed repository checkout operation")
        ))
        # Verify that the actual git_source message "checking out" doesn't match
        self.assertEqual("other", downloader.classify_repository_failure(
            RepositorySyncError("Failed checking out branch")
        ))
        self.assertEqual("other", downloader.classify_repository_failure(RuntimeError("unknown")))
        self.assertEqual("api", downloader.classify_repository_failure(KeyError("missing")))
        self.assertEqual("api", downloader.classify_repository_failure(TypeError("type")))
        self.assertEqual("api", downloader.classify_repository_failure(ValueError("value")))

    def test_get_default_branch_name_extracts_mainbranch(self):
        """Test that get_default_branch_name extracts mainbranch.name."""
        repo = {"mainbranch": {"name": "master"}}
        self.assertEqual("master", downloader.get_default_branch_name(repo))

    def test_get_default_branch_name_extracts_default_branch_dict(self):
        """Test that get_default_branch_name extracts default_branch.name."""
        repo = {"default_branch": {"name": "main"}}
        self.assertEqual("main", downloader.get_default_branch_name(repo))

    def test_get_default_branch_name_extracts_default_branch_string(self):
        """Test that get_default_branch_name extracts default_branch as string."""
    ...
    def test_get_default_branch_name_returns_none_for_missing(self):
        """Test that get_default_branch_name returns None when branch info missing."""
        self.assertIsNone(downloader.get_default_branch_name({}))
        self.assertIsNone(downloader.get_default_branch_name(None))
        self.assertIsNone(downloader.get_default_branch_name({"other": "field"}))

    def test_select_working_branches_returns_empty_for_empty_input(self):
        """Test that select_working_branches handles empty branch list."""
    ...
    def test_select_working_branches_returns_empty_for_default_only_without_name(self):
        """Test that select_working_branches returns empty when default_branch_only but no name."""
        branches = [{"name": "main"}, {"name": "dev"}]
        result = downloader.select_working_branches(
            branches=branches,
    ...
    def test_select_working_branches_returns_all_when_no_filters(self):
        """Test that select_working_branches returns all branches when no filters."""
    ...
    def test_select_working_branches_skips_branches_without_name(self):
        """Test that select_working_branches skips branches without a name field."""
    ...
    def test_format_failure_counters_returns_none_for_zero_failures(self):
        """Test that format_failure_counters returns 'none' when no failures."""
        counters = downloader.make_failure_counters()
        self.assertEqual("none", downloader.format_failure_counters(counters))

    def test_format_failure_counters_formats_multiple_types(self):
        """Test that format_failure_counters formats multiple failure types."""
        counters = {"api": 2, "auth": 1, "clone": 0, "fetch": 3, "checkout": 0, "other": 0}
        result = downloader.format_failure_counters(counters)
        self.assertIn("api=2", result)
        self.assertIn("auth=1", result)
        self.assertIn("fetch=3", result)
        self.assertNotIn("clone=0", result)

    def test_list_backup_entries_returns_empty_for_nonexistent_dir(self):
        """Test that list_backup_entries returns empty list for nonexistent directory."""
        entries = downloader.list_backup_entries("/nonexistent/path")
        self.assertEqual([], entries)

    def test_list_backup_entries_skips_non_directory_files(self):
        """Test that list_backup_entries skips non-directory files in output dir."""
        with tempfile.TemporaryDirectory() as tmp_dir:
            # Create a file instead of a directory
            file_path = os.path.join(tmp_dir, "not-a-dir.txt")
    ...
    def test_run_backup_skips_archived_repository_by_default(self):
        """Test that run_backup skips archived repositories unless include_archived is True."""
        args = SimpleNamespace(
            workspace=None,
            role="member",
            include_archived=False,
        ...
        class _FakeProviderWithArchived(_FakeProvider):
            def get_repository(self, workspace, name):
                repo = super().get_repository(workspace, name)
                repo["archived"] = True
                return repo

        provider = _FakeProviderWithArchived()
        git_source = _FakeGitSource()
        logger = _MemoryLogger()
        ...
        self.assertEqual(0, stats["succeeded"])
        skip_events = [event for event in logger.events if event["action"] == "repository.skip"]
        self.assertEqual("archived", skip_events[0]["reason"])

    def test_run_backup_skips_repository_without_ssh_clone_url(self):
        """Test that run_backup skips repositories without SSH clone URL."""
        args = SimpleNamespace(
            workspace=None,
            role="member",
            include_archived=False,
        ...
        class _FakeProviderWithoutSSH(_FakeProvider):
            def get_repository(self, workspace, name):
                repo = super().get_repository(workspace, name)
                repo["links"]["clone"] = [{"name": "https", "href": "https://example.com/repo.git"}]
                return repo

        provider = _FakeProviderWithoutSSH()
        git_source = _FakeGitSource()
        logger = _MemoryLogger()
        ...
        self.assertEqual(1, stats["skipped"])
        skip_events = [event for event in logger.events if event["action"] == "repository.skip"]
        self.assertEqual("missing_ssh_clone_url", skip_events[0]["reason"])

    def test_run_backup_working_mode_skips_checkout_when_no_branches_match(self):
        """Test that working mode skips checkout when no branches match selectors."""
        args = SimpleNamespace(
            workspace=None,
            role="member",
            include_archived=False,
            ...
            include=[],
            exclude=[],
            branch_names=["nonexistent"],
            branch_patterns=[],
            default_branch_only=False,
            repo_retries=0,
        )
        provider = _FakeProvider(branches=[{"name": "main"}, {"name": "dev"}])
        git_source = _FakeGitSource()
        logger = _MemoryLogger()
        ...
        self.assertEqual(1, stats["succeeded"])
        checkout_calls = [call for call in git_source.calls if call[0] == "checkout_branch"]
        self.assertEqual(0, len(checkout_calls))
        skip_events = [
            event for event in logger.events if event["action"] == "sync.working.branches.skip"
        ]
        self.assertEqual(1, len(skip_events))
